...
public class Math3d {
	...
	public static bool IsLineInRectangle(Vector3 linePoint1, Vector3 linePoint2, List<cArea> rect){
		
		bool pointAInside = false;
		bool pointBInside = false;
		
		pointAInside = gpsPosition.PlayerInsideArea(linePoint1,rect);	//IsPointInRectangle(linePoint1,rect);
		
		if(!pointAInside){
			
			pointBInside = gpsPosition.PlayerInsideArea(linePoint2,rect);	//IsPointInRectangle(linePoint2, rect);
		}
		...
		if(!pointAInside && !pointBInside){
			...
			bool isLineCrossing = false;

//			for(int i=0; i<rect.Count-1; i++){
//
//				List<Vector3> simeia = rect[i].Simeia;
//				
//
//				if(linePoint1 != rect[i] && linePoint1 != rect[i+1] && linePoint2 != rect[i] && linePoint2 != rect[i+1])
//				{
//					isLineCrossing = AreLineSegmentsCrossing(linePoint1, linePoint2, rect[i], rect[i+1]);
//
//					if(isLineCrossing){
//						return true;
//					}
...
//			}
	...
	public static bool IsPointInRectangle(Vector3 point, List<Vector3> rect){
		
		Vector3 vector;
		Vector3 linePoint;
		int indxLast = rect.Count-1;
		int indxMiddle = Mathf.FloorToInt(rect.Count/2);
		
		///get the center of the rectangle
		vector = rect[indxLast] - rect[0];
		float size = -(vector.magnitude / 2f);
		vector = AddVectorLength(vector, size);
		Vector3 middle = rect[0] + vector;
		
		Vector3 xVector = rect[indxMiddle] - rect[0];
		float width = xVector.magnitude / 2f;
		
		Vector3 yVector = rect[indxLast] - rect[0];
		float height = yVector.magnitude / 2f;
