...
namespace UnityEngine.UI.Extensions
{
	...
	public class DropDownList : MonoBehaviour
	{
		...
		public Text textButn;

		public void Start()
		{
			Initialize();

			if(transform.name.Contains("Languange")){
				if(Diadrasis.Instance.languangeNow==Diadrasis.LanguangeNow.gr){
					textButn.text="ΕΠΙΛΟΓΗ";
					Items[0].Caption="ΕΛΛΗΝΙΚΑ";
					Items[1].Caption="ΑΓΓΛΙΚΑ";
				}else{
					textButn.text="SELECT";
					Items[0].Caption="GREEK";
					Items[1].Caption="ENGLISH";
				}
			}
		}
		...
		*/
		...
		private void OnItemClicked(int indx)
		{
			...
			if(transform.name.Contains("Languange")){
				if(indx==0){
					SendMessageUpwards("SetLanguange","gr");
				}else
				if(indx==1){
					SendMessageUpwards("SetLanguange","en");
				}
				if(Diadrasis.Instance.languangeNow==Diadrasis.LanguangeNow.gr){
					textButn.text="ΕΠΙΛΟΓΗ";
					Items[0].Caption="ΕΛΛΗΝΙΚΑ";
