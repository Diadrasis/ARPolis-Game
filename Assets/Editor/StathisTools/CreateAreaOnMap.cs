...
//		currentArea = new cArea();
	...
	public static void ExportAllAreasPaths(){
		...
		List<GameObject> perioxesMenuScenes = new List<GameObject>();
		...
		foreach(GameObject g in gs){
			...
			if(g.name=="Area_ForScene"){
				perioxesMenuScenes.Add(g);
			}
		}
		...
		for(int t=0; t<5; t++)
		{
			...
			if(t==4){
				myAreas = perioxesMenuScenes;
				//if no areas dont write anything - skip
				if(myAreas.Count>0){
					sw.Write(xmlHeaderSceneAreas());
				}else{continue;}
			}
...
//				Debug.LogWarning("perioxisPoints = "+perioxisPoints.Count);
	...
	static string xmlHeaderSceneAreas(){
		StringBuilder s = new StringBuilder();
		s.AppendLine("<ActiveAreaForScene>");
		return s.ToString();	
	}

	static string xmlFooterSceneAreas(){
		StringBuilder s = new StringBuilder();
		s.AppendLine("</ActiveAreaForScene>");
		return s.ToString();	
	}
