...
public class TextureDetails : IEquatable<TextureDetails>
{
    ...
    public bool Equals(TextureDetails other)
    {
        return texture != null && other.texture != null &&
               texture.GetNativeTextureID() == other.texture.GetNativeTextureID();
    }

    public override int GetHashCode()
    {
        return texture.GetNativeTextureID();
    }
...
public class ResourceChecker : EditorWindow {
	...
	[MenuItem ("Diadrasis/Έλεγχος Πόρων Σκηνής")]
	static void Init ()
	{  
	...
	void OnGUI ()
	{
        ...
        if (GUILayout.Button("CleanUp")) Resources.UnloadUnusedAssets();
	...
	void ListTextures()
	{
		...
		{			
			...
			if(GUILayout.Button(tDetails.texture.name,GUILayout.Width(250)))
			{
			...
			sizeLabel+=" - "+tDetails.mipMapCount+" mipMapCount";
			sizeLabel+=" - "+FormatSizeString(tDetails.memSizeKB)+" - "+tDetails.format+"";

			GUILayout.Label (sizeLabel,GUILayout.Width(280));

			if(GUILayout.Button("Χρησιμοποιείται απο "+tDetails.FoundInMaterials.Count+" Materials",GUILayout.Width(250)))
			{
			...
			if (GUILayout.Button(FoundObjects.Count+" >> Επιλογή στη σκηνή",GUILayout.Width(180)))
			{
		...
		if (ActiveTextures.Count>0)
		{
			...
			if(GUILayout.Button("Επιλογή όλων",GUILayout.Width(150)))
			{
	...
	void CheckResources()
	{
		...
		ActiveTextures.Sort(delegate(TextureDetails details1, TextureDetails details2) { return details1.onoma.CompareTo(details2.onoma);  }); //(delegate(TextureDetails details1, TextureDetails details2) { return details2.memSizeKB - details1.memSizeKB; });
		...
		ActiveMaterials.Sort (delegate(MaterialDetails det1, MaterialDetails det2) {
			return det1.material.name.CompareTo (det2.material.name);
		});
	...
	private T[] FindObjects<T>() where T : Object
	{
		if (IncludeDisabledObjects)
		{
		    ...
		    return objects.Where(x => PrefabUtility.GetPrefabType(x) != PrefabType.Prefab && PrefabUtility.GetPrefabType(x) != PrefabType.ModelPrefab).ToArray();
		    //return objects;
		}
		else
		{
	...
	private TextureDetails GetTextureDetail(Texture tTexture)
	{
		...
		if (tTextureDetails == null)
		{
			...
			tTextureDetails.onoma = tTexture.name;
