...
public class ExtraTools : EditorWindow {
	...
	[MenuItem("Diadrasis/Build AssetBundle From Selection - List Names (texture - sounds)")]
	...
	[MenuItem("Diadrasis/Build AssetBundle From Selection - No dependency tracking (texture - sounds)")]
	...
	[MenuItem("Diadrasis/Build AssetBundle From Selection - Track dependencies (prefab)")]
	...
	[MenuItem("Diadrasis/Clear Project Cache")]
	...
	[MenuItem ("Diadrasis/Extra Tools")]
	static void Init () {
	...
	void RandomSpawnItems()
	{
		...
		if(fatherContainer)
		{
			...
			if(itemsToSpawn>0 && prefabGb)
			{
				...
				{
					Vector3 newPos = new Vector3(Random.Range(minX,maxX),initPos.y,Random.Range(minZ,maxZ));
					...
					if(posForSpawn.Count>0)
					{
						for(int s=0; s<posForSpawn.Count; s++)
						{
							...
							{
								newPos = new Vector3(Random.Range(minX,maxX),initPos.y,Random.Range(minZ,maxZ));
...
//		Text[] txts = FindObjectsOfType<Text>();
	...
	void Screenshot(){
		if(myKam){
			Application.CaptureScreenshot("Map_"+System.DateTime.Now.ToString("yyyy_MM_dd HH.mm.ss")+".png",mySize);
...
//		}
	...
	void GetAllTextures(){
		Object[] allTX = Resources.FindObjectsOfTypeAll(typeof(Texture2D));
